...
use Nzolt\NetSuite\WebServices\Platform\Core\RecordRef;
use Nzolt\NetSuite\WebServices\Platform\Messages\Preferences;
use Nzolt\NetSuite\WebServices\Platform\Messages\SearchPreferences;
...
class NSPHPClient {
    ...
    protected function __construct($wsdl=null, $options=array()) {
        ...
        if (!isset($wsdl)) {
             if (!array_key_exists('nshost', $options)) {
                throw new \Exception('Webservice host must be specified');
             }
             if (!array_key_exists('nsendpoint', $options)) {
                throw new \Exception('Webservice endpoint must be specified');
             }
             $wsdl = $options['nshost'] . "/wsdl/v" . $options['nsendpoint'] . "_0/netsuite.wsdl";
        }
        ...
        if ( $this->generated_from_endpoint != $options['nsendpoint'] ) {
            // check for the endpoint compatibility failed, but it might still be compatible. Issue only warning
            $endpoint_warning = 'The NetSuiteService classes were generated from the '.$this->generated_from_endpoint .' endpoint but you are running against ' . $options['nsendpoint'];
        ...
        $options['location'] = $options['nshost'] . "/services/NetSuitePort_" . $options['nsendpoint'];
        ...
        if (array_key_exists('debuginfo', $options)) {
            $httpheaders .= "\r\nDebug: true";
            $httpheaders .= "\r\nUser: " . $options['debuginfo']["email"];
            $httpheaders .= "\r\nPassword: " . $options['debuginfo']["password"];
            $httpheaders .= "\r\nIssue: " . $options['debuginfo']["issue"];
        }

        $options['user_agent'] =  $httpheaders;
        $this->setPassport($options['nsaccount'], $options['nsemail'], $options['nsrole'], $options['nspassword']);
    ...
    public function setPassport($nsaccount, $nsemail, $nsrole, $nspassword) {
        ...
        $this->passport->setAccount($nsaccount);
        $this->passport->setEmail($nsemail);
        $this->passport->setPassword($nspassword);
        $this->passport->role = new RecordRef();
        $this->passport->role->setInternalId($nsrole);
    }
    ...
    public function setSearchPreferences ($bodyFieldsOnly = true, $pageSize = 50, $returnSearchColumns = true)
    {
        ...
        $sp->setBodyFieldsOnly($bodyFieldsOnly);
        $sp->setPageSize($pageSize);
        $sp->setReturnSearchColumns($returnSearchColumns);
